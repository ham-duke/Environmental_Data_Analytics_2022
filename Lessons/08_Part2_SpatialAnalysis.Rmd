...
#can also save these from viewer by clicking export can make it an html file 
```
...
huc8_sf.subest <- st_read('./Data/Spatial/NCHUC8.shp')%>%
  filter(SUBBASIN == 'Upper Neuse')

#Create a map

ggplot() + 
  geom_sf(data=huc8_sf.subest, fill = 'purple')+
  geom_sf(data=huc8_sf, fill = 'orange', alpha = 0.1)
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC 12' = 'factor')) %>% 
  mutate(HUC_8 = substr(HUC_12,1,8)) %>%
  group_by(HUC_8)%>%
  summarise(SumWindEnergy = sum(AvgWindEnergy))

#Join to HUC_8 features

HUC_8_join <- merge(x = huc8_sf,
                    y = wind_raw,
                    by.x = "HUC_8",
                    by.y = "HUC_8")


HUC_8_sf_join2 <- huc8_sf %>%
  left_join(wind_raw, by = c("HUC_8" = "HUC_8"))


#View the outputs

ggplot(data = HUC_8_sf_join2)+
