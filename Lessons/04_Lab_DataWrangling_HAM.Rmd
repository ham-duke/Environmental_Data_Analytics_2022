...
NTL.phys.data.surface
#to maybe only get surface temp or other vars. 
...
unique(NTL.phys.data$year4)
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999)
unique(NTL.phys.data.1999$year4)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
class(NTL.phys.data$lakename)
NTL.phys.data.Tuesday.1990.1999 <- filter(NTL.phys.data, lakename %in% ("Tuesday Lake"), year4 %in% c(1990:1999))
unique(NTL.phys.data.Tuesday.1990.1999$year4)


#Can also select certain columns using select function and mutate function 
#mutate allows adding new columns as a result of an existing column (like a conversion of temp from F to C )
```
...
colnames(NTL.phys.data.Tuesday.1990.1999)
NTL.phys.data.Tuesday.1990.1999.July <- NTL.phys.data.Tuesday.1990.1999 %>% filter(month(sampledate)== 7)  
dim(NTL.phys.data.Tuesday.1990.1999.July)
...
NTL.phys.data.Tuesday.1990.1999.July.meansurface <- NTL.phys.data.Tuesday.1990.1999.July %>% 
  filter( depth == 0)%>%
  filter(!is.na(temperature_C))%>% #returns logical opperator T/F therefore the ! makes it all obs that are not NA thats what we are taking 
