...
> Add notes here: climate change 

```{r, message = FALSE}
getwd()
setwd("/Users/Dell Laptop/Documents/Github/EDA")
#install.packages(tidyverse)
library(tidyverse)
#install.packages(lubridate)
library(lubridate)
NTL.phys.data <- read.csv("~/GitHub/EDA/Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)#summary will work for character entries with stringasfactor
...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y")#4 digit year is Y
...
`>=`
`&` and to both be true
`|` or one or the other to be true 
...
unique(NTL.phys.data.Tuesday.1990.1999$lakename)
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)#ascending order

NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth))#descending order
