...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T),    #What does "na.rm=T" do? Is it necessary? #remove nas True
...
#make simple table into simple feature 
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'),
           crs=4326)
#long = x, lat = y, crs = 4326... (well explain later...)
class(EPAair.sf)
mapview(EPAair.sf)
...
#This is a simple feature point
...
espg.io - can learn what coordinate system something is 
...
Say we want to determine the distance between our EPA sites and the nearest Gas Plant. We don't want to report the distances in degrees, so we first want to project our data into a projected crs. We'll use the UTM Zone 17 N - WGS84 crs (EPSG = 32617) 
